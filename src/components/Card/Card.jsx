...
function Card({ data }) {
    ...
    const notFoundMessage = 'Not mentioned';
    return (
        <div className="w-fit bg-white space-y-4 shadow-lg rounded-lg p-4 md:p-6">
            <div className="flex flex-col justify-start items-start space-y-4  md:items-center md:space-x-4 md:flex-row">
            ...
            <div className="">
                <p>Email: {data.email ? data.email : notFoundMessage}</p>
                <p>Active since: {data.created_at.substring(0, 10)}</p>
                <p>Hireable: {(data.hireable) ? data.hireable : notFoundMessage}</p>
                <p>Location: {(data.location) ? data.location : notFoundMessage}</p>
